...
public class GameController implements Runnable {
	...
	public GameController(final GameView view) {
		...
		this.keyListener = new KeyAdapter() {
			...
			public void keyPressed(final KeyEvent event) {
				if(!(updateInterval > 0)){
					if (isRunning) updateGame(event.getKeyCode());
				}
				else enqueueKeyPress(event.getKeyCode());
			}
		};
	...
	public void startGame(final GameModel gameModel) {
		...
		// Create the new thread and start it if updateInterval is larger than 0.
	...
	public void updateGame(final int keyPress){
		try {
